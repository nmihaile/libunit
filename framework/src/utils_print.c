...
/*   Updated: 2025/09/02 11:06:58 by nmihaile         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	status_msg[9][32] = {LIGHTGREEN"[OK]"DEFAULT, \
		LIGHTRED"[KO]"DEFAULT, \
		LIGHTYELLOW"[SEGV]"DEFAULT, \
		LIGHTYELLOW"[BUSE]"DEFAULT, \
		LIGHTYELLOW"[ABRT]"DEFAULT, \
		LIGHTYELLOW"[SFPE]"DEFAULT, \
...
void	ftu_print_final_result(void)
{
	if (ftu_count_routine_result(0, 0, false))
	{
		...
		ft_putstr_fd("================================================", 1);
		ft_putendl_fd(DEFAULT, 1);
		ft_putstr_fd("  🎉🥳"LIGHTGREEN BOLD" All tests passed!", 1);
		ft_putendl_fd(REGULAR GREEN" Congratulations! 🥳🎉  ", 1);
		ft_putstr_fd(GREEN, 1);
		ft_putstr_fd("================================================", 1);
		ft_putendl_fd(DEFAULT, 1);
		return ;
